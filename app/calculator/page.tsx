  ...
  return (
    ...
    >
      ...
      <div className="space-y-5">
        ...
        {/* 1. Audience size */}
        <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4">
          <label className="block text-sm font-semibold text-[#191715] mb-1">
            Размер аудитории
          </label>
          <p className="text-[11px] text-[#8A8078] mb-3">
          ...
          <div className="flex items-center gap-3 mb-3">
            <input
              ...
              className="w-full rounded-xl border border-[#E8E2DA] bg-white/[0.05] px-4 py-2.5 text-center text-lg font-bold text-[#B08D3E] outline-none transition focus:border-[#B08D3E]/50 focus:ring-1 focus:ring-[#B08D3E]/30"
            />
          </div>
          ...
          <div className="relative">
            <input
              ...
              style={{
                background: `linear-gradient(to right, #B08D3E 0%, #B08D3E ${sliderPercent}%, rgba(255,255,255,0.1) ${sliderPercent}%, rgba(255,255,255,0.1) 100%)`,
                WebkitAppearance: "none",
              }}
            />
            {/* Custom thumb via global style below */}
          </div>
          <div className="flex justify-between mt-1.5 text-[10px] text-[#8A8078]">
            <span>100</span>
            <span>100 000</span>
          </div>
        </div>
              ...
              <button
                ...
                className={`rounded-xl px-3 py-2 text-sm font-semibold transition-all duration-200 ${
                  price === p && !isCustomPrice
                    ? "bg-[#B08D3E] text-[#FFFFFF] shadow-lg shadow-[#B08D3E]/20"
                    : "border border-[#E8E2DA] bg-white/[0.05] text-[#57534E] hover:border-[#B08D3E]/40 hover:text-[#191715]"
                }`}
              >
                {fmt(p)}&nbsp;₽
              </button>
          ...
          <div className="relative">
            <input
              ...
              className={`w-full rounded-xl border px-4 py-2.5 text-sm font-semibold outline-none transition ${
                isCustomPrice
                  ? "border-[#B08D3E]/50 bg-[#B08D3E]/10 text-[#B08D3E] ring-1 ring-[#B08D3E]/30"
                  : "border-[#E8E2DA] bg-white/[0.05] text-[#78716C] placeholder:text-[#D6D3D1]"
              } focus:border-[#B08D3E]/50 focus:ring-1 focus:ring-[#B08D3E]/30`}
            />
            <span className="absolute right-4 top-1/2 -translate-y-1/2 text-sm text-[#8A8078]">
              ₽
            </span>
          </div>
        </div>
            ...
            {LAUNCH_TYPES.map((lt) => (
              <button
                ...
                className={`rounded-xl p-3 text-left transition-all duration-200 ${
                  launchType === lt.key
                    ? "border-2 border-[#B08D3E] bg-[#B08D3E]/10 shadow-lg shadow-[#B08D3E]/10"
                    : "border border-[#E8E2DA] bg-white/[0.03] hover:border-[#E8E2DA]"
                }`}
              >
                <div
                  className={`text-sm font-bold ${
                    launchType === lt.key ? "text-[#B08D3E]" : "text-[#191715]"
                  }`}
                >
                  {lt.label}
                </div>
                <div className="text-[11px] text-[#8A8078] mt-0.5">
        ...
        <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4">
          <div className="flex items-center justify-between">
            <div>
              <label className="block text-sm font-semibold text-[#191715]">
                Был опыт продаж?
              </label>
        ...
        <button
          ...
          className="pulse-gold w-full rounded-2xl bg-gradient-to-r from-[#B08D3E] to-[#D4B96A] py-4 text-base font-extrabold text-[#FFFFFF] transition-all duration-200 hover:shadow-lg hover:shadow-[#B08D3E]/30 active:scale-[0.98] disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {isCalculating ? (
      ...
      {isCalculating && (
        <div className="mt-6 flex flex-col items-center gap-3 py-8 fade-in-up">
          <div className="relative h-16 w-16">
            <div className="absolute inset-0 animate-ping rounded-full bg-[#B08D3E]/20" />
            <div className="absolute inset-2 animate-pulse rounded-full bg-[#B08D3E]/30" />
            <div className="absolute inset-0 flex items-center justify-center">
              <svg className="w-8 h-8 text-[#B08D3E] animate-spin" fill="none" viewBox="0 0 24 24">
      ...
      {showResults && (
        <div ref={resultsRef} className="mt-6 space-y-4 fade-in-up">
          ...
          <div className="rounded-2xl border border-[#B08D3E]/20 bg-gradient-to-br from-[#B08D3E]/10 to-transparent p-4">
            <h3 className="text-sm font-bold text-[#191715] mb-4 text-center">
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="rounded-xl bg-white/[0.04] p-3 text-center">
                <div className="text-[10px] font-semibold uppercase tracking-wider text-[#8A8078] mb-1.5">
                  Пессимист.
                </div>
                <div className="text-lg font-extrabold text-[#57534E] leading-tight">
                  {fmt(animPessimistic)}
                </div>
                <div className="text-[10px] text-[#8A8078] mt-0.5">₽</div>
              </div>

              {/* Realistic */}
              <div className="rounded-xl border border-[#B08D3E]/30 bg-[#B08D3E]/10 p-3 text-center relative overflow-hidden">
                <div className="absolute inset-0 bg-gradient-to-b from-[#B08D3E]/5 to-transparent" />
                <div className="relative">
                  <div className="text-[10px] font-semibold uppercase tracking-wider text-[#B08D3E] mb-1.5">
                    Реалистич.
                  </div>
                  <div className="text-xl font-extrabold text-[#B08D3E] leading-tight">
                    {fmt(animRealistic)}
                  </div>
                  <div className="text-[10px] text-[#B08D3E]/60 mt-0.5">₽</div>
                </div>
              </div>
          ...
          <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4">
            <h3 className="text-sm font-bold text-[#191715] mb-3">
              Расчёт
            </h3>
            <div className="flex items-center justify-center gap-1 flex-wrap text-sm">
              <span className="rounded-lg bg-white/[0.06] px-2.5 py-1.5 font-bold text-[#191715]">
                {fmt(audience)}
              </span>
              <span className="text-[#8A8078]">&times;</span>
              <span className="rounded-lg bg-white/[0.06] px-2.5 py-1.5 font-bold text-[#B08D3E]">
              ...
              <span className="rounded-lg bg-[#B08D3E]/15 px-2.5 py-1.5 font-extrabold text-[#B08D3E]">
                {fmt(animRealistic)}&nbsp;₽
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4 text-center">
              <div className="text-[11px] font-semibold uppercase tracking-wider text-[#8A8078] mb-1">
                Заявок нужно
              </div>
              <div className="text-2xl font-extrabold text-[#B08D3E]">
                {fmt(animLeads)}
              </div>
            ...
            <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4 text-center">
              ...
              <div className="text-2xl font-extrabold text-[#D4B96A]">
                {lt.timeRange}
              </div>
              <div className="text-[10px] text-[#8A8078] mt-1">
          ...
          <div className="rounded-2xl border border-[#E8E2DA] bg-white/[0.03] p-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-[#B08D3E]">С наставником</span>
                <span className="text-xs font-bold text-[#B08D3E]">{fmt(animMentor)} ₽</span>
              </div>
              <div className="h-7 w-full rounded-full bg-white/[0.06] overflow-hidden">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-[#B08D3E] to-[#D4B96A] transition-all duration-1000 ease-out relative"
              ...
              <div className="text-center mt-2">
                <span className="inline-flex items-center gap-1 rounded-full bg-[#B08D3E]/15 px-3 py-1 text-[11px] font-bold text-[#B08D3E]">
          ...
          <div className="rounded-2xl border border-[#B08D3E]/15 bg-gradient-to-r from-[#B08D3E]/[0.07] to-transparent p-4">
            <h3 className="text-sm font-bold text-[#191715] mb-2">
            ...
            <div className="flex items-center gap-3">
              <div className="flex-1 rounded-xl bg-white/[0.05] p-3 text-center">
                <div className="text-[10px] text-[#8A8078] mb-1">Было</div>
                <div className="text-base font-extrabold text-[#78716C]">{fmt(animRealistic)} ₽</div>
              </div>
              <div className="shrink-0">
                <svg className="w-5 h-5 text-[#B08D3E]" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5 21 12m0 0-7.5 7.5M21 12H3" />
                </svg>
              </div>
              <div className="flex-1 rounded-xl border border-[#B08D3E]/30 bg-[#B08D3E]/10 p-3 text-center">
                <div className="text-[10px] text-[#B08D3E]/60 mb-1">Стало</div>
                <div className="text-base font-extrabold text-[#B08D3E]">{fmt(animRaised)} ₽</div>
              </div>
            </div>
            <div className="text-center mt-2">
              <span className="text-[11px] text-[#8A8078]">
                Разница: <span className="font-bold text-[#B08D3E]">+{fmt(revRaised - revRealistic)} ₽</span>
              </span>
            </div>
          </div>

          {/* Bottom CTA */}
          <div className="rounded-2xl border border-[#B08D3E]/25 bg-gradient-to-br from-[#B08D3E]/15 to-[#FFFFFF] p-5 text-center">
            <p className="text-base font-bold text-[#191715] mb-1">
              Хочешь получить не прогноз,
            </p>
            <p className="text-base font-bold text-[#191715] mb-1">
              а реальные <span className="text-[#B08D3E]">300К за 6 недель</span>?
            </p>
            <p className="text-[11px] text-[#8A8078] mt-2 mb-4">
            ...
            <a
              ...
              className="pulse-gold inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#B08D3E] to-[#D4B96A] px-7 py-3 text-sm font-extrabold text-[#FFFFFF] transition hover:shadow-lg hover:shadow-[#B08D3E]/30"
            >
          ...
          <button
            ...
            className="w-full rounded-xl border border-[#E8E2DA] bg-white/[0.03] py-3 text-sm font-semibold text-[#78716C] transition hover:border-[#E8E2DA] hover:text-[#57534E]"
          >
      ...
      <style>{`
        input[type="range"]::-webkit-slider-thumb {
          ...
          height: 22px;
          border-radius: 50%;
          background: linear-gradient(135deg, #D4B96A, #B08D3E);
